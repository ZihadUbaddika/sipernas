...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('dashboard.admin.users_management.roles.store') }}" method="POST"
                ...
                <div class="form-group {{ $errors->has('tipe') ? 'has-error' : '' }}">
                    <label for="tipe">{{ trans('global.roles') }}<span
                ...
                <div class="form-group {{ $errors->has('wilayah_id') ? 'has-error' : '' }}">
                    <label for="wilayah_id">{{ trans('id.pengajuan.tabel.wilayah') }}<span
                            class="text-danger">*</span></label>
                    <select class="form-control select {{ $errors->has('wilayah_id') ? 'is-invalid' : '' }}" name="wilayah_id"
                        ...
                        @foreach (App\Models\UserManagement\Role::wilayah_select as $key => $wilayah)
                                <option value="{{ $key }}"
                                    {{ old('wilayah', '') === (string) $key ? 'selected' : '' }}>{{ $wilayah }}
                                </option>
                        @endforeach
                    </select>
                </div>
