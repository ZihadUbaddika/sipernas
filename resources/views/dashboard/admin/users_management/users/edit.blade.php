...
<div class="card">
    <div class="card-header">
        {{ trans('id.ubah') }} {{ trans('id.user.judul_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("dashboard.admin.users_management.users.update", [$user->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="form-group {{ $errors->has('kepegawaian_id') ? 'has-error' : '' }}">
                ...
                <select name="kepegawaian_id" id="kepegawaian_id" class="form-control select2bs4">
                    ...
                    @foreach($kepegawaian_id as $id => $kepegawaian_id)
                        <option value="{{ $id }}" {{ $user->kepegawaian_id == $id ? 'selected' : '' }}>
                            {{ $kepegawaian_id }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group clearfix">
                <div class="icheck-danger d-inline">
                  <input type="checkbox" id="checkboxPrimary1" name="change_password">
                  <label for="checkboxPrimary1">Ubah password ?</label>
                </div>
            </div>
            <div class="form-group {{ $errors->has('roles') ? 'has-error' : '' }}">
                <label for="roles">{{ trans('id.user.tabel.roles') }}*</label>
